...
Restart the OBC.
#### Reason
We use SPI protocol to program the micro-controller so after programming the OBC the buffer gets partially filled and we get garbage output.
Restarting the OBC clears the buffer.

### Programmer led red while programming the slave.
#### Solution
Erase the master chip first.
